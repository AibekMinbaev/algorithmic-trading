...
class TestApp(EClient, EWrapper):
    ...
    def accountSummary(self, reqId, account, tag, value, currency):
        ...
        if self.account_balance is not None:
            self.reqHistoricalData(
                self.orderId, self.contract, "", "12 M", "1 day", "MIDPOINT", 0, 1, False, [])
    ...
    def historicalDataEnd(self, reqId, start, end):
        ...
        self.ma_200 = df['200d_EMA'].iloc[-1]

        print("200 EMA: ", self.ma_200)
    ...
    def onPriceUpdate(self, price):
        ...
        # Do it 
        if self.macd > self.signal and price > self.ma_200 and self.current_position == 0:
            print("Buying")
            self.placeBuyOrder(self.orderId, self.contract)

        elif self.macd < self.signal and price < self.ma_200 and self.current_position > 0:
            print("Selling")
    ...
    def placeBuyOrder(self, orderId, contract):
        ...
        # Risk management: 0.001 of current_balance
        amount_to_use = self.account_balance * 0.001
        ...
        if 'last' in self.current_prices:
            ...
            print(
                f"Bought {quantity} of {self.contract.symbol} for {amount_to_use} USD")
